 ...
 * \ingroup CAN
 * \name canFD_SSP_xxx
 * \anchor canFD_SSP_xxx
 *
 * Flags for \ref canFdSecondarySamplePoint().
 *
 * @{
 */
/** Secondary sample point is automatically adjusted dynamically. */
#define canFD_SSP_AUTO   0
/** Set secondary sample point. */
#define canFD_SSP_SET    1
/** Get secondary sample point. */
#define canFD_SSP_GET    2
/** @} */

/**
 * \ingroup CAN
 *
 * \source_cs       <b>static Canlib.canStatus canFdSecondarySamplePoint(int hnd, ref int ns, int mode);</b>
 *
 * \source_delphi   <b>function canFdSecondarySamplePoint(hnd: canHandle; var ns: Cardinal; mode: Cardinal): canStatus;</b>
 * \source_end
 *
 * This function sets or retrieves the secondary sampling point for
 * the specified CAN FD controller.
 *
 * If \a mode is canFD_SSP_AUTO, the \a ns argument is ignored, and
 * the secondary sampling point is automatically adjusted dynamically.
 * If \a mode is canFD_SSP_SET, the secondary sampling point is set to
 * \a ns, and if \a mode is canFD_SSP_GET, it is retrived and stored
 * in \a ns.
 *
 * \param[in]     hnd     An open handle to a CAN controller.
 * \param[in,out] ns      Secondary sampling point delay, in nanoseconds.
 * \param[in]     mode    One of canFD_SSP_AUTO, canFD_SSP_GET, or
 *                        canFD_SSP_SET.
 *
 * \return \ref canOK (zero) if success
 * \return \ref canERR_xxx (negative) if failure
 */
canStatus CANLIBAPI canFdSecondarySamplePoint(const CanHandle hnd,
                                              unsigned int *ns,
                                              int mode);
 ...
 * \source_end
 *
 * The \ref canSetBitrate() function sets the bit rate of the specified CAN
...
//ĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤĤč
//
// APIs for t-script
//
